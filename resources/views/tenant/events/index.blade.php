<x-app-layout>
    <x-slot name="header">
        ...
        <a href="{{ route('tenant.events.create') }}" class="text-sm text-blue-600 hover:text-blue-900">Create Event</a>
    </x-slot>
        ...
        <div class="max-w-5xl mx-auto sm:px-6 lg:px-8">
            <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg">
                <div class="p-6">
                                ...
                                @foreach($events as $event)
                                    <tr>
                                        ...
                                        <td class="px-6 py-4 whitespace-nowrap text-right text-sm font-medium">
                                            <a href="{{ route('tenant.events.edit', $event) }}" class="text-indigo-600 hover:text-indigo-900 mr-4">Manage</a>
                                            <form action="{{ route('tenant.events.destroy', $event) }}" method="POST" class="inline">
                                                @csrf @method('DELETE')
                                                <button class="text-red-600 hover:text-red-900" onclick="return confirm('Delete event?')">Delete</button>
                                            </form>
                                        </td>
                                    </tr>
                                @endforeach
                    ...
                    @else
                        <p class="text-center py-4 text-gray-500">No events found.</p>
                    @endif
                </div>
            </div>
        </div>
