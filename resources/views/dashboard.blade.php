<x-app-layout>
    ...
    <div class="py-12">
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
            ...
            <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg p-6 mb-8 border-l-4 {{ $tenant->isConnectedToStripe() ? 'border-green-500' : 'border-yellow-500' }}">
                <div class="flex flex-col md:flex-row justify-between items-center gap-4">
                    <div>
                        <h3 class="text-lg font-bold text-gray-900">{{ __('Payment Configuration') }}</h3>
                        
                        @if($tenant->isConnectedToStripe())
                            <p class="text-gray-500 text-sm mt-1">
                                {{ __('Your Stripe account is connected. You can accept payments.') }}
                            </p>
                        @else
                            <p class="text-gray-500 text-sm mt-1">
                                {{ __('Connect your Stripe account to start selling tickets.') }}
                            </p>
                        @endif
                    </div>
                    ...
                    <div>
                        @if($tenant->isConnectedToStripe())
                            <div class="inline-flex items-center px-4 py-2 bg-green-100 border border-green-200 rounded-md font-semibold text-xs text-green-700 uppercase tracking-widest">
                                <svg xmlns="http://www.w3.org/2000/svg" class="h-4 w-4 mr-2" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                        ...
                        @else
                            <a href="{{ route('tenant.stripe.connect') }}" class="inline-flex items-center px-4 py-2 bg-indigo-600 border border-transparent rounded-md font-semibold text-xs text-white uppercase tracking-widest hover:bg-indigo-700 active:bg-indigo-900 focus:outline-none focus:border-indigo-900 focus:ring ring-indigo-300 disabled:opacity-25 transition ease-in-out duration-150">
                                {{ __('Connect with Stripe') }} &rarr;
                            </a>
                        @endif
                    </div>
                </div>
